<template>
	<div>
		...
		<div class="top-info">
			<div class="left-info">
				<view class="avatar">
					<image src="/static/avatar.jpeg" style="width:50px;height:50px" fade-show mode="widthFix">
					</image>
				</view>
				<div class="nick">
			...
			<div class="right-info">
				<button v-if="!imgInfo.hasFocus" :disabled="isMyself" size="mini" class="focus"
					@click="focus(imgInfo.pUserId)">+关注</button>
				<button v-else size="mini" class="focus" @click="openDialog">已关注</button>
			</div>
		</div>
		...
		<TkScreenScroll :lengths="length" @getScreenIndes="getScreenIndes" @toLeft="toLeft" @toRight="toRight">
						...
						<view class="chat-custom-right">
							...
							<view class="reply">
								...
								<uni-icons class="icon" type="chat" color="#69B197" size="20"
									@click="replyUser(comment)"></uni-icons>
			...
			<view class="bottom">
				<view class="comInput">
					<uni-search-bar v-model="myComment" bgColor="#fff" placeholder="发表评论" @confirm="comment"
						cancelButton="none" clearButton="always">
			...
			<view>
				...
				<uni-popup ref="alertDialog" type="dialog">
					<uni-popup-dialog type="warn" cancelText="取消" confirmText="确定" content="确认取消关注该用户吗？"
						@confirm="cancelFocus"></uni-popup-dialog>
				</uni-popup>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async focus(focusUserId) {
				let infoObj = {
					userId: this.userInfo.id,
					focusUserId
				}
				let res = await uni.$API.user.reqFocusUser(infoObj)
				console.log(res);
				if (res.data.code == 200) {
					this.getPicInfo()
				}
			...
			async cancelFocus(focusUserId) {
				let infoObj = {
					userId: this.userInfo.id,
					focusUserId: this.imgInfo.pUserId
				}
				let res = await uni.$API.user.reqCancelFocus(infoObj)
				console.log(res);
				if (res.data.code == 200) {
					this.getPicInfo()
				}
